    ...
    <section id="activities" className="bg-black py-24 px-4 md:px-8 border-t border-white/5 relative z-10 overflow-hidden">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="relative w-full flex flex-col gap-6">
          ...
          <div className="absolute left-[23px] top-6 bottom-6 w-[2px] bg-foreground/5 pointer-events-none" />
                ...
                <div className="relative z-10 flex flex-col items-center mr-4 md:mr-6 flex-shrink-0">
                  <button
                    ...
                    style={{
                      ...
                      boxShadow: isOpen ? '0 0 15px var(--accent-shadow)' : 'none'
                    }}
                  >
                    ...
                    <div className="relative w-6 h-6 flex items-center justify-center">
                      ...
                      <div className="relative flex items-center justify-center w-full h-full">
                        <div 
                          ...
                          style={{ backgroundColor: isOpen ? 'var(--primary-color)' : 'var(--text-muted)' }}
                        />
                        <div 
                          className="absolute h-3 w-0.5 rounded-full transition-all duration-500"
                          style={{
                            backgroundColor: isOpen ? 'transparent' : 'var(--text-muted)',
                  ...
                  <div 
                    ...
                    style={{
                      background: isOpen ? 'linear-gradient(to bottom, var(--primary-color), transparent)' : 'transparent',
                      opacity: isOpen ? 1 : 0,
                ...
                <div className="flex-1 min-w-0">
                  ...
                  >
                    <span 
                      ...
                      style={{ color: isOpen ? 'var(--text-color)' : 'var(--text-muted)' }}
                    >
                      {item.title}
                    </span>
                  </button>
                  ...
                  <AnimatePresence initial={false}>
                    {isOpen && (
                      ...
                      >
                        <div className="p-6 bg-muted/65 backdrop-blur-sm border-x border-b border-foreground/5 rounded-b-2xl relative">
                          ...
                          {item.points && item.points.length > 0 && (
                            <ul className="space-y-3.5">
                              {item.points.map((pt, pIdx) => (
                                <li key={pIdx} className="flex items-start gap-3 text-xs sm:text-sm text-foreground/80 font-light leading-relaxed">
                          ...
                          {item.subitems && item.subitems.length > 0 && (
                            <div className="flex flex-col gap-3">
                              {item.subitems.map((sub, sIdx) => (
                                <div key={sIdx} className="flex justify-between items-center border-b border-foreground/5 pb-2.5 last:border-b-0 last:pb-0 gap-4">
                                  <div className="flex items-center gap-2.5 min-w-0">
                                    <Award className="w-4 h-4 text-primary shrink-0" />
                                    <span className="text-xs sm:text-sm text-foreground/80 truncate">{sub.label}</span>
                                  </div>
