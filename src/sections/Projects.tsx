...
function ProjectCard({ project, index }: { project: Project; index: number }) {
  ...
  return (
    <motion.article
      ...
      className="group relative shrink-0 overflow-hidden rounded-3xl glass-strong flex flex-col w-[290px] h-[380px] sm:w-[350px] sm:h-[420px] md:w-[390px] md:h-[450px] lg:w-[420px] lg:h-[470px]"
    >
      <div className="relative aspect-[16/10] overflow-hidden shrink-0">
        ...
        <div className="absolute inset-x-0 bottom-0 p-5">
          <span className="font-display text-2xl sm:text-3xl font-semibold leading-tight text-gradient line-clamp-2">
            {project.title}
          </span>
        </div>
        <div className="absolute right-3 top-3 rounded-full bg-black/60 px-2.5 py-1 text-[10px] uppercase tracking-[0.25em] text-white/90 backdrop-blur">
          0{index + 1}
        </div>
      </div>

      <div className="p-6 flex flex-col flex-grow justify-between min-h-0">
        <div className="space-y-4 overflow-hidden flex-grow flex flex-col justify-center">
          <p className="text-xs sm:text-sm text-muted-foreground line-clamp-3 leading-relaxed">{project.description}</p>

          <ul className="flex flex-wrap gap-1.5 overflow-hidden max-h-[64px]">
            {project.tech.map((t) => (
              <li
                key={t}
                className="rounded-full border border-white/10 bg-white/[0.03] px-2.5 py-1 text-[9px] sm:text-[10px] uppercase tracking-wider text-muted-foreground shrink-0"
              >
                {t}
              </li>
            ))}
