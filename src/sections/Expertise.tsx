                  ...
                  <svg className="w-full h-full" viewBox="0 0 100 50">
                    ...
                    {[
                      ...
                      <motion.circle 
                        ...
                        animate={{ scale: [0.8, 1.25, 0.8], opacity: [0.5, 0.95, 0.5] }}
...
export const Expertise: React.FC = () => {
  ...
  const accentColor = themeMode === 'light' ? "#000000" : "#DEDBC8"; // Primary gold/black
  const lineColor = themeMode === 'light' ? "rgba(0, 0, 0, 0.08)" : "rgba(222, 219, 200, 0.08)"; // Faded line
  const cornerMaskColor = themeMode === 'light' ? "#F9F9FB" : "#000000"; // Background mask matching theme
  const lineWidth = 4;
  const dotSize = 16;
  const showDots = true;
      ...
      {!isFirst && (
        <div
          style={{
            ...
            clipPath: reversed ? "inset(0px 4px 0px 0px)" : "inset(0px 0px 0px 4px)"
          }}
        />
      )}
      {!isLast && (
